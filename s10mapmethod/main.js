let products = [
  ...
  */
const MXNprice = (products,dll) =>{
  let result = products.map((item)=>{
    let {id, title, price, description, category, image, rating,} = item
    let mxnPrice = item.price * dll
    price += ` Dlls / ${mxnPrice.toFixed(2)} MXN.`
      return {id, title, price , description, category, image, rating,}
  })
  console.log(result);
}

MXNprice(products,18.08)
...
const smDescription = (products) =>{
  let result = products.map((item)=>{
    let {id, title, price, description, category, image, rating,} = item
    let newSmTxt = `${description.split(" ").splice(0,10).join(" ")}...`
    description = newSmTxt
       return {id, title, price , description, category, image, rating,}
  })
  console.log(result);
}

smDescription(products)
